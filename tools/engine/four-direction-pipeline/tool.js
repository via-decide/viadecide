...
const STORE_KEY = 'engine.four-direction-pipeline.draft';
...
function runEngine() {
  ...
  const result = EngineModels.four_direction_pipeline(metrics, parsed, secondary);

  output.textContent = JSON.stringify({
    tool: 'four-direction-pipeline',
    purpose: 'structure four-direction input',
    mode,
    metrics,
    result,
...
document.getElementById('download').addEventListener('click', () => {
  ...
  a.download = 'four-direction-pipeline-output.json';
  a.click();
});
