...
const STORE_KEY = 'engine.trust-score-engine.draft';
...
function runEngine() {
  ...
  const result = EngineModels.trust_score_engine(metrics, parsed, secondary);

  output.textContent = JSON.stringify({
    tool: 'trust-score-engine',
    purpose: 'compute trust score',
    mode,
    metrics,
    result,
...
document.getElementById('download').addEventListener('click', () => {
  ...
  a.download = 'trust-score-engine-output.json';
  a.click();
});
