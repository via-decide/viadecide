...
const STORE_KEY = 'engine.orchard-discovery-search.draft';
...
function runEngine() {
  ...
  const result = EngineModels.orchard_discovery_search(metrics, parsed, secondary);

  output.textContent = JSON.stringify({
    tool: 'orchard-discovery-search',
    purpose: 'search orchard summaries',
    mode,
    metrics,
    result,
...
document.getElementById('download').addEventListener('click', () => {
  ...
  a.download = 'orchard-discovery-search-output.json';
  a.click();
});
