...
const STORE_KEY = 'engine.fruit-sharing.draft';
...
function runEngine() {
  ...
  const result = EngineModels.fruit_sharing(metrics, parsed, secondary);

  output.textContent = JSON.stringify({
    tool: 'fruit-sharing',
    purpose: 'share completed outputs',
    mode,
    metrics,
    result,
...
document.getElementById('download').addEventListener('click', () => {
  ...
  a.download = 'fruit-sharing-output.json';
  a.click();
});
