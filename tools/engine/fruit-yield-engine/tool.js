...
const STORE_KEY = 'engine.fruit-yield-engine.draft';
...
function runEngine() {
  ...
  const result = EngineModels.fruit_yield_engine(metrics, parsed, secondary);

  output.textContent = JSON.stringify({
    tool: 'fruit-yield-engine',
    purpose: 'convert completed work to fruits',
    mode,
    metrics,
    result,
...
document.getElementById('download').addEventListener('click', () => {
  ...
  a.download = 'fruit-yield-engine-output.json';
  a.click();
});
