...
const STORE_KEY = 'engine.circle-builder.draft';
...
function runEngine() {
  ...
  const result = EngineModels.circle_builder(metrics, parsed, secondary);

  output.textContent = JSON.stringify({
    tool: 'circle-builder',
    purpose: 'build circles/communities',
    mode,
    metrics,
    result,
...
document.getElementById('download').addEventListener('click', () => {
  ...
  a.download = 'circle-builder-output.json';
  a.click();
});
