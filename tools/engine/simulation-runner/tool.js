...
const STORE_KEY = 'engine.simulation-runner.draft';
...
function runEngine() {
  ...
  const result = EngineModels.simulation_runner(metrics, parsed, secondary);

  output.textContent = JSON.stringify({
    tool: 'simulation-runner',
    purpose: 'run multi-player simulations',
    mode,
    metrics,
    result,
...
document.getElementById('download').addEventListener('click', () => {
  ...
  a.download = 'simulation-runner-output.json';
  a.click();
});
