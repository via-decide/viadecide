...
const STORE_KEY = 'engine.meta-health-dashboard.draft';
...
function runEngine() {
  ...
  const result = EngineModels.meta_health_dashboard(metrics, parsed, secondary);

  output.textContent = JSON.stringify({
    tool: 'meta-health-dashboard',
    purpose: 'meta health monitoring',
    mode,
    metrics,
    result,
...
document.getElementById('download').addEventListener('click', () => {
  ...
  a.download = 'meta-health-dashboard-output.json';
  a.click();
});
