...
const STORE_KEY = 'engine.player-signup.draft';
...
function runEngine() {
  ...
  const result = EngineModels.player_signup(metrics, parsed, secondary);

  output.textContent = JSON.stringify({
    tool: 'player-signup',
    purpose: 'create player identity and starter archetype',
    mode,
    metrics,
    result,
...
document.getElementById('download').addEventListener('click', () => {
  ...
  a.download = 'player-signup-output.json';
  a.click();
});
