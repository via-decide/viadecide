...
const STORE_KEY = 'engine.hire-readiness-scorer.draft';
...
function runEngine() {
  ...
  const result = EngineModels.hire_readiness_scorer(metrics, parsed, secondary);

  output.textContent = JSON.stringify({
    tool: 'hire-readiness-scorer',
    purpose: 'compute hire readiness',
    mode,
    metrics,
    result,
...
document.getElementById('download').addEventListener('click', () => {
  ...
  a.download = 'hire-readiness-scorer-output.json';
  a.click();
});
