...
const STORE_KEY = 'engine.fair-ranking-engine.draft';
...
function runEngine() {
  ...
  const result = EngineModels.fair_ranking_engine(metrics, parsed, secondary);

  output.textContent = JSON.stringify({
    tool: 'fair-ranking-engine',
    purpose: 'compute fair ranking without volume bias',
    mode,
    metrics,
    result,
...
document.getElementById('download').addEventListener('click', () => {
  ...
  a.download = 'fair-ranking-engine-output.json';
  a.click();
});
