...
const STORE_KEY = 'engine.growth-path-recommender.draft';
...
function runEngine() {
  ...
  const result = EngineModels.growth_path_recommender(metrics, parsed, secondary);

  output.textContent = JSON.stringify({
    tool: 'growth-path-recommender',
    purpose: 'recommend next best task',
    mode,
    metrics,
    result,
...
document.getElementById('download').addEventListener('click', () => {
  ...
  a.download = 'growth-path-recommender-output.json';
  a.click();
});
